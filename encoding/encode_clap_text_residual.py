#!/usr/bin/env python3
"""
MEG Encoding — CLAP Text Residual Ridge Regression (GPU)
=========================================================
Identical to meg_encoding_clap_gpu.py but uses residual features:
  clap_text_residual_wav2vec.npy  (CLAP text with Wav2Vec removed)

This is the key analysis from Figure 4 (left side) of the paper:
  - If predictivity drops at ~200ms after removal → speech info confirmed
  - Compare output against meg_predictions/clap_text_L*/ to see the drop

Input:
  notebooks/clap_text_residual_wav2vec.npy  -- residual features, 12 layers
  notebooks/sub{01-27}-meg-data.npy         -- preprocessed MEG, 27 subjects

Output:
  meg_predictions/clap_text_residual_L{00-11}/
    corr_sub{01-27}.npy   -- Pearson r, shape (16848,)
    r2_sub{01-27}.npy     -- R2,        shape (16848,)

Usage:
  conda activate meg_env
  nohup taskset -c 0-15 python3 -u meg_encoding_clap_residual_gpu.py > clap_residual_encoding_log.txt 2>&1 &
  echo $!
"""
...
FEATURE_FILE  = NOTEBOOKS_DIR / "clap_text_residual_wav2vec.npy"
...
def encode_subject_layer(sub, layer_idx, features, data, out_dir):
    ...
    for train_folds, test_fold in kf.split(np.arange(4)):
        # Residual features are already stacked arrays (n_words, 768)
...
def main():
    print("=" * 60)
    print("MEG Encoding — CLAP Text Residual (GPU)")
    ...
    if not FEATURE_FILE.exists():
        ...
        print("Run compute_residuals.py first.")
        return

    print(f"\nLoading residual features...")
    features = np.load(str(FEATURE_FILE), allow_pickle=True)
    print(f"  Word counts: {[features[s][0].shape[0] for s in range(4)]}")
    print(f"  Feature dim: {features[0][0].shape[1]}")
    ...
    for layer_idx in range(N_LAYERS):
        ...
        out_dir = PRED_DIR / f"clap_text_residual_L{layer_idx:02d}"
    ...
    print(f"Results: {PRED_DIR}/clap_text_residual_L*/")
    print("\nNext: compare corr curves vs clap_text_L*/ to see 200ms drop")
