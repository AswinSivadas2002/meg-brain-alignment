#!/usr/bin/env python3
"""
MEG Encoding — CLAP Text Layer-wise Ridge Regression (GPU)
===========================================================
Same as meg_encoding_bert_gpu.py but for CLAP text features.
Run AFTER clap_feature_extraction.py has completed.

Input:
  notebooks/clap_text_features.npy    -- CLAP text features, 12 layers
  notebooks/sub{01-27}-meg-data.npy   -- preprocessed MEG, 27 subjects

Output:
  meg_predictions/clap_text_L{00-11}/
    corr_sub{01-27}.npy   -- Pearson r, shape (16848,)
    r2_sub{01-27}.npy     -- R2,        shape (16848,)

Everything else identical to BERT encoding:
...
Usage:
  ...
  nohup taskset -c 0-15 python meg_encoding_clap_gpu.py > clap_encoding_log.txt 2>&1 &
  echo $!
"""
...
FEATURE_FILE  = NOTEBOOKS_DIR / "clap_text_features.npy"
...
# Lambda grid — same as BERT (paper Appendix E)
LAMBDAS = np.logspace(1, 3, 16).astype(np.float32)

# Voxel chunk size — keeps GPU memory under 25GB
GPU_VOXEL_CHUNK = 4000

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

# ─── GPU RIDGE REGRESSION (identical to BERT script) ─────────────────────────
...
def cross_val_ridge_gpu(X_train, Y_train):
    ...
    Returns: weights (n_feat, n_voxels), best_lambdas (n_voxels,)
    """
...
def main():
    print("=" * 60)
    print("MEG Encoding — CLAP Text (GPU)")
    ...
    if not FEATURE_FILE.exists():
        ...
        print("Run clap_feature_extraction.py first.")
        return

    print(f"\nLoading CLAP text features...")
    features = np.load(str(FEATURE_FILE), allow_pickle=True)
    print(f"  Word counts per story: {[features[s][0].shape[0] for s in range(4)]}")
    print(f"  Feature dim (L0):      {features[0][0].shape[1]}")
    ...
    for layer_idx in range(N_LAYERS):
        ...
        out_dir = PRED_DIR / f"clap_text_L{layer_idx:02d}"
    ...
    print(f"Results saved to: {PRED_DIR}/clap_text_L*/")
