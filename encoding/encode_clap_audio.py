#!/usr/bin/env python3
"""
MEG Encoding — CLAP Audio Layer-wise Ridge Regression (GPU)
============================================================
Encodes raw CLAP audio features against MEG brain data.
CLAP audio uses HTSAT encoder with 5 stages (variable channel dims).

Input:
  notebooks/clap_audio_features_synth.npy    -- CLAP audio, 5 layers
  notebooks/sub{01-27}-meg-data.npy    -- preprocessed MEG, 27 subjects

  Layer dims (confirmed from model inspection):
    L0: (n_words, 96)
    L1: (n_words, 192)
    L2: (n_words, 384)
    L3: (n_words, 768)
...
Output:
  meg_predictions/clap_audio_synth_L{00-04}/
    corr_sub{01-27}.npy   -- Pearson r, shape (16848,)
    r2_sub{01-27}.npy     -- R2,        shape (16848,)

Usage:
  conda activate meg_env
  nohup taskset -c 0-15 python3 -u meg_encoding_clap_audio_gpu.py > clap_audio_encoding_log.txt 2>&1 &
  echo $!
"""
...
FEATURE_FILE  = NOTEBOOKS_DIR / "clap_audio_features_synth.npy"

N_LAYERS   = 5    # HTSAT has 5 stages
N_SUBJECTS = 27
...
def main():
    print("=" * 60)
    print("MEG Encoding — CLAP Audio (GPU)")
    print(f"  Features: {FEATURE_FILE.name}")
    print(f"  Subjects: {N_SUBJECTS}  Layers: 0-{N_LAYERS-1} (HTSAT stages)")
    ...
    for layer_idx in range(N_LAYERS):
        ...
        out_dir = PRED_DIR / f"clap_audio_synth_L{layer_idx:02d}"
    ...
    print(f"Results: {PRED_DIR}/clap_audio_synth_L*/")
