#!/usr/bin/env python3
"""
MEG Encoding — Wav2Vec2 Layer-wise Ridge Regression (GPU)
==========================================================
Unimodal speech baseline for Figure 3 comparison.

Input:
  notebooks/wav2vec_features.npy    -- 12 layers, 768-dim
  notebooks/sub{01-27}-meg-data.npy -- preprocessed MEG

Output:
  meg_predictions/wav2vec_L{00-11}/
    corr_sub{01-27}.npy   -- Pearson r, shape (16848,)
    r2_sub{01-27}.npy     -- R2,        shape (16848,)

Usage:
  nohup taskset -c 0-15 python3 -u meg_encoding_wav2vec_gpu.py > wav2vec_encoding_log.txt 2>&1 &
  echo $!
"""
...
FEATURE_FILE  = NOTEBOOKS_DIR / "wav2vec_features.npy"
...
def encode_subject_layer(sub, layer_idx, features, data, out_dir):
    ...
    for train_folds, test_fold in kf.split(np.arange(4)):
        x_train = np.concatenate(
            [np.array(features[i][layer_idx]) for i in train_folds],
        ...
        y_test  = data[test_fold[0]].reshape(
            len(data[test_fold[0]]), N_VOXELS).astype(np.float32)
...
def main():
    print("=" * 60)
    print("MEG Encoding — Wav2Vec2 (GPU)")
    ...
    print(f"  Word counts: {[len(features[s][0]) for s in range(4)]}")
    print(f"  Feature dim: {np.array(features[0][0]).shape[1]}")
    ...
    for layer_idx in range(N_LAYERS):
        ...
        out_dir = PRED_DIR / f"wav2vec_L{layer_idx:02d}"
        out_dir.mkdir(parents=True, exist_ok=True)

        for sub in SUBJECTS:
            meg_path = NOTEBOOKS_DIR / f"sub{sub}-meg-data.npy"
            if not meg_path.exists():
                print(f"    sub{sub}: MEG not found, skipping")
                continue
            t_sub = time.time()
        ...
        print(f"Layer {layer_idx:02d} done in {(time.time()-t_layer)/60:.1f} min")

    print(f"\nAll done in {(time.time()-t_total)/3600:.2f} hours")
    print(f"Results: {PRED_DIR}/wav2vec_L*/")
